...
// ---------------------------------------------------------
// 🛡️ 错误边界防御系统 (Error Boundary)
// 防止任何组件级别的 JS 错误导致全屏黑屏
// ---------------------------------------------------------
      ...
      return (
        <div className="min-h-screen bg-[#0d1117] text-white p-10 flex flex-col items-center justify-center font-mono">
          <div className="bg-red-900/50 border border-red-500 p-8 rounded-xl max-w-3xl w-full shadow-2xl">
            ...
            <p className="mt-6 text-xs text-white/40 italic">// 请将上述错误截图发给 AI 进行排查修复。</p>
          </div>
        </div>
      );
    }
...
const MathDisplay = ({ tex, katexReady }) => {
  ...
  if (error) {
    return <div className="my-4 text-xs text-red-500 bg-red-900/20 p-4 border border-red-500/50 rounded text-center">Formula Error: {error}</div>;
  }
...
for i, r in enumerate(r_values):
    ...
    if i == 0: ax_cobweb.legend(loc='upper left')
        
    ax_time = axes[i, 1]
    ax_time.plot(traj, 'g.-', lw=1, alpha=0.6)
    ax_time.set_title(f'Time Series: r={r}')
    ax_time.set_xlim(0, iters); ax_time.set_ylim(0, 1)
    ax_time.grid(True, linestyle='--', alpha=0.5)
    
    meanings = "Fixed Point" if r == 2.8 else "2-Cycle" if r == 3.2 else "Chaos"
    ...
    <LogLayout title="DYNAMICAL_SYSTEMS: THE_TOPOLOGY_OF_CHAOS" category="RESEARCH" date="2026-03-26">
      <div className="space-y-12 font-mono text-white/80 text-sm md:text-base leading-relaxed max-w-5xl mx-auto pb-20">
        ...
        <section className="space-y-4">
          ...
          {/* 🛡️ 关键修复：使用 String.raw 语法防止反斜杠被 JS 引擎吞掉 */}
          <MathDisplay tex={String.raw`|x_{n+1} - x^*| \approx |f'(x^*)| \cdot |x_n - x^*|`} katexReady={katexReady} />
        ...
        <section className="space-y-6">
          ...
          {/* 🛡️ 关键修复：String.raw 保护复杂公式 */}
          <MathDisplay tex={String.raw`\delta = \lim_{n \to \infty} \frac{r_n - r_{n-1}}{r_{n+1} - r_n} \approx 4.6692016...`} katexReady={katexReady} />
        ...
        <section className="space-y-4">
          ...
          {/* 🛡️ 关键修复：String.raw 保护复杂公式 */}
          <MathDisplay tex={String.raw`Sf(x) = \frac{f'''(x)}{f'(x)} - \frac{3}{2} \left( \frac{f''(x)}{f'(x)} \right)^2 < 0`} katexReady={katexReady} />
...
// 用 ErrorBoundary 包裹整个组件导出
const ChaosLogWrapped = () => (
  <ErrorBoundary>
    <ChaosLogContent />
  </ErrorBoundary>
);

export default ChaosLogWrapped;